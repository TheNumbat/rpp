...
template<Allocator A, u64 Buckets, u64 Bias>
Range_Allocator<A, Buckets, Bias>::Range_Allocator(u64 heap_size) {
    assert(heap_size);
...
template<Allocator A, u64 Buckets, u64 Bias>
Range_Allocator<A, Buckets, Bias>::~Range_Allocator() {
    Thread::Lock lock(mutex);
    reset();
}

template<Allocator A, u64 Buckets, u64 Bias>
Range_Allocator<A, Buckets, Bias>::Range_Allocator(Range_Allocator&& src) {
    *this = std::move(src);
}

template<Allocator A, u64 Buckets, u64 Bias>
Range_Allocator<A, Buckets, Bias>&
Range_Allocator<A, Buckets, Bias>::operator=(Range_Allocator<A, Buckets, Bias>&& src) {
...
template<Allocator A, u64 Buckets, u64 Bias>
void Range_Allocator<A, Buckets, Bias>::reset() {
...
template<Allocator A, u64 Buckets, u64 Bias>
void Range_Allocator<A, Buckets, Bias>::Stats::assert_clear() {
...
template<Allocator A, u64 Buckets, u64 Bias>
void Range_Allocator<A, Buckets, Bias>::insert_free_block(
    Range_Allocator<A, Buckets, Bias>::Block* block) {
    block->free = true;
...
template<Allocator A, u64 Buckets, u64 Bias>
void Range_Allocator<A, Buckets, Bias>::remove_free_block(
    Range_Allocator<A, Buckets, Bias>::Block* block) {
    block->free = false;
    u8 idx = size_to_idx(block->size);
...
template<Allocator A, u64 Buckets, u64 Bias>
u8 Range_Allocator<A, Buckets, Bias>::size_to_idx(u64 size) {
...
template<Allocator A, u64 Buckets, u64 Bias>
Opt<typename Range_Allocator<A, Buckets, Bias>::Range>
Range_Allocator<A, Buckets, Bias>::allocate(u64 size, u64 alignment) {
...
template<Allocator A, u64 Buckets, u64 Bias>
void Range_Allocator<A, Buckets, Bias>::free(Range_Allocator<A, Buckets, Bias>::Range allocation) {
...
template<Allocator A, u64 Buckets, u64 Bias>
typename Range_Allocator<A, Buckets, Bias>::Stats Range_Allocator<A, Buckets, Bias>::statistics() {
    Stats copy;
    {
        Thread::Lock lock(mutex);
        copy = stats;
...
template<Allocator A, u64 Buckets, u64 Bias>
void Range_Allocator<A, Buckets, Bias>::test() {
    ...
    for(u64 i = 0; i < 1000; i++) {
        ...
        for(u64 j = 0; j < 10000; j++) {
            u64 align = rng.range(1ul, Math::MB(1));
            u64 size = rng.range(1ul, Math::MB(1));
            auto mem = allocator.allocate(size, align).get();
            assert(mem->offset % align == 0);
            allocations.push(mem);
        }
        rng.shuffle(Slice{allocations});
