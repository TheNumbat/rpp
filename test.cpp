...
i32 main() {
    ...
    //             i32 i = job(99, 0b10).block(); // has to run on another thread, blocks this
    //             thread
    //             // continue on same thread, wait does not swap out
    //             info("5.1: co_await 1ms job");
    //             auto j = co_await job(1, 0); // likely runs on this thread as we yield
    //             immediately
    //             // should continue on same thread because we had time to install the continuation
    //             info("5.1: launch 0s job");
    //             auto wait =
    //                 job(0, 0); // should run on another thread, we don't yield until the next
    //                 await
